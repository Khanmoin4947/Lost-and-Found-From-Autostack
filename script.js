...
function setReportType(type) {
  ...
  if (labelLocationKind)
    labelLocationKind.textContent = isLost ? "lost" : "found";
}
...
function validateReportForm() {
  ...
  if (fieldContactName && !fieldContactName.value.trim()) {
    ...
    if (errorContactName)
      errorContactName.textContent = "Contact name is required.";
  }
  ...
  if (fieldContactEmail && fieldContactEmail.value) {
    ...
    if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(value)) {
      ...
      if (errorContactEmail)
        errorContactEmail.textContent = "Enter a valid email address.";
    }
  }
...
if (reportForm) {
  reportForm.addEventListener("submit", async (evt) => {
    ...
    formData.append("status", reportStatusInput?.value === "found" ? "found" : "lost");
    formData.append("title", fieldTitle?.value.trim() || "");
    formData.append("category", fieldCategory?.value || "");
    formData.append("description", fieldDescription?.value.trim() || "");
    formData.append("date", fieldDate?.value || "");
    formData.append("location", fieldLocation?.value.trim() || "");
    formData.append("contactName", fieldContactName?.value.trim() || "");
    formData.append("contactEmail", fieldContactEmail?.value.trim() || "");
    formData.append("contactPhone", fieldContactPhone?.value.trim() || "");

    if (fieldImageFile?.files[0]) {
      formData.append("image", fieldImageFile.files[0]);
    }
...
// intialistion
loadItems();       // now async — it calls populateCategoryFilter + renderItems internally
showPage("home");